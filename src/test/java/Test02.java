...
/**
 * 思路：采用自己写的堆来实现 大小为k 之所以不使用系统提供的，是因为无法在时间复杂度logN内自由删除一个元素
 * private ArrayList<Info> heap; // 用来保存长度为k的子数组
 * private ArrayList<Info> heapCopy; // 用来保持长度为k的子数组，元素顺序和原数组arr保持一致，用来提供每次从heap中取出最大元素之后需要删除的元素
 * private HashMap<Info, Integer> indexMap;  // 记录堆中每个数据节点在heap中的位置 key:节点 value：位置
 *
 */
public class Test02 {

    public static void kSubArrayMax(int[] arr, int k) {
...
//            List<Integer> list1 = kSubArrayMax(arr, k);
//            List<Integer> list2 = test(arr, k);
